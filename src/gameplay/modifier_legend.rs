...
/// `note_anim_mode` / `note_shape_params` the falling notes use, so the legend
...
/// regular `NoteShapeMaterial`s, so `animate_note_tails` drives them in time with
...
pub fn build_legend_materials(
    materials: &mut Assets<NoteShapeMaterial>,
) -> Vec<(Handle<NoteShapeMaterial>, &'static str)> {
    ...
    legend_techniques()
        ...
        .map(|(i, (modifier, name))| {
            ...
            let (mut params, mut wah_v) = note_shape_params(PREVIEW_H_PCT, vib, shift, wah);
            ...
            let handle = materials.add(NoteShapeMaterial {
                color,
                params,
                wah: wah_v,
            });
...
pub fn spawn_modifier_legend(
    ...
    font: &FontSource,
    entries: &[(Handle<NoteShapeMaterial>, &'static str)],
) {
    parent
        .spawn(Node {
