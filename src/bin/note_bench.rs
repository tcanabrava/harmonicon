...
//! phantom summary plus the most common confusion pairs.
//!
...
//!
//! Usage: `cargo run --bin note_bench [-- <path>]`, defaulting to
//! `assets/debug_songs`.
...
use harmonicon::note_bench::{compare, expected_notes_from_chart, run_algorithm};
use harmonicon::song::chart::HarpChart;
use std::path::{Path, PathBuf};

fn main() {
    let root = std::env::args().nth(1).unwrap_or_else(|| "assets/debug_songs".to_string());
    ...
    for song_dir in &song_dirs {
        ...
        ran_any = true;
        run_one(song_dir, &chart_path, &wav_path);
    }

    if !ran_any {
        println!(
            "No debug recordings found under {} yet. Record one via the Song \
             Editor's Debug Recording checkbox (cargo run --features dev), \
...
fn run_one(song_dir: &Path, chart_path: &Path, wav_path: &Path) {
    let name = song_dir
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("?");
