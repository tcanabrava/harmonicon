...
Lessons may embed focused practice tools below their instructional text. The
manifest owns only declarative configuration; reusable rendering and behavior
live in `harmonicon-ui`, and `harmonicon-lessons` hosts each widget without
depending on gameplay state.

1. Add a schema-validated, tagged `LessonWidget` list. Start with the existing
   circle of fifths and retain `diagram` as a compatibility input for external
   lessons. Convert bundled content to `widgets` once the new path works.
2. Extract the reusable 12-bar presentation from `harmonicon-gameplay` into
   `harmonicon-ui`. Gameplay continues to supply its current bar; a lesson can
   supply a key, progression, and optional local highlighted bar.
3. Split the metronome into a shared view/model and clock adapters. Gameplay
   keeps using `GameplayClock`; lessons get an entity-local start/stop clock so
   multiple widgets cannot leak state into one another and audio stops on exit.
4. Add lesson controls: key/position selection for the circle, bar stepping for
   the grid, and start/stop, BPM, feel, and mute for the metronome. An optional
   `sync_group` lets a metronome advance a 12-bar grid every configured number
   of beats.
5. Pilot the widgets in the circle-of-fifths, bar-counting, and early rhythm
   lessons. Widget interaction remains instructional; existing chart, jam, and
   Mark-as-Done paths remain the only completion authorities.

Validation follows the same boundary as the implementation: schema tests catch
authoring errors; pure tests cover circle/transposition, ticks, bar advancement,
wrapping, and synchronization; a few Bevy tests cover spawning, `Activate`
interaction, independent widget state, and cleanup. Update
`docs/gameplay_validation.md` when metronome timing is extracted.
